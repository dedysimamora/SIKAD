...
const InputDataForm = (props) => {
  ...
  const handleSubmit = () => {
    ...
    if (isBase64(formik.values.foto, { allowMime: true })) {
      ...
      const uploadImage = storage
        .ref(`images/image-${formik.values.noDefinitif}`)
        .putString(formik.values.foto, "data_url");
      uploadImage.on(
        "state_changed",
        (snapshoot) => {},
        (error) => {
  ...
  useEffect(() => {
    dispatch.arsip.getAllArsip(DynamicData["SIKAD"].title.toUpperCase());
  }, []);
            ...
            >
              ...
              <Row style={{ marginTop: "20px" }}>
                <Col span={12} style={{ flexDirection: "row" }}>
                  <PanjangFoto formik={formik} />
                </Col>
                <Col
                  span={12}
                  style={{ display: "flex", flexDirection: "row" }}
                >
                  <LebarFoto formik={formik} />
                </Col>
              </Row>
        ...
        <Col md={{ span: 12 }} xs={{ span: 24 }}>
          {!isMobile && (
            <Row style={{ marginBottom: "10px" }}>
              <Col span={12}>
                <p className={"noDefinitif"}>
                  No Definitif :{" "}
                  {formik.values.noDefinitif !== "" ? (
                    formik.values.noDefinitif
                  ) : (
                    <span>-</span>
                  )}
          ...
          <Row gutter={16}>
            <Col md={{ span: 8 }} xs={{ span: 24 }}>
              <NoKlasifikasi formik={formik} />
            </Col>
            <Col md={{ span: 8 }} xs={{ span: 24 }}>
              <TanggalSimpan formik={formik} />
            </Col>
            <Col md={{ span: 8 }} xs={{ span: 24 }}>
              <TipeArsip formik={formik} />
            </Col>
          </Row>
          <Row gutter={16}>
            <Col md={{ span: 12 }} xs={{ span: 24 }}>
              <HakCipta formik={formik} />
            </Col>
            <Col md={{ span: 12 }} xs={{ span: 24 }}>
              <LokasiTempat formik={formik} />
            </Col>
          </Row>
          <Row gutter={16}>
            <Col md={{ span: 12 }} xs={{ span: 24 }}>
              <Keterangan formik={formik} />
            </Col>
            <Col md={{ span: 12 }} xs={{ span: 24 }}>
              <TempatSimpan formik={formik} />
            </Col>
          </Row>
          <Row>
            <Col span={24}>
              <Indeks formik={formik} />
            </Col>
          </Row>
          <Row>
            <Col span={24}>
              <UraianInformasi formik={formik} />
            </Col>
          </Row>
        </Col>
